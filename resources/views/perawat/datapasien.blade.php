<table id="datapasien" class="table table-sm text-sm table-bordered table-hover">
    ...
    <tbody>
        @foreach ($pasien as $p)
        <tr onclick="location.href='erm/{{$p->kode_kunjungan}}'" class="pilihpasien toastsDefaultSuccess" nomor-rm="{{ $p->no_rm }}" nama="{{ $p->nama }}" kodekunjungan="{{ $p->kode_kunjungan }}" alamat="{{ $p->alamat }}" counter="{{ $p->counter }}" umur="{{ $p->umur }}" unit="{{ $p->unit }}" tglmasuk="{{ $p->tgl_masuk }}">
